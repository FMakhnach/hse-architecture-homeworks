...
namespace glob {
	...
	// Mutex for controlling result_set access.
...
// Checking every k * i + thread_num value (this way every thread will calculate its part).
void ProcessNums(int thread_num) {
	for (int i = glob::LOWER_BOUND + thread_num; i < glob::UPPER_BOUND; i += glob::num_of_threads) {
		CheckNum(i);
	}
}
...
bool TryReadNumberOfThreads() {
	...
	return 0 < glob::num_of_threads && glob::num_of_threads <= std::thread::hardware_concurrency();
}
